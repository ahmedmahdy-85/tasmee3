...
class StudentReviewViewModel extends GetxController {
  String groupName;
  bool clickable = true;
  ...
  void onInit() async {
    ...
    await getGroupName();
    await getStudentsName();
  }

  void setDropDownItem(String value) {
    name = value;
    userDetails.clear();
    dateFrom.clear();
    dateUntil.clear();
  ...
  Future<void> getStudentStatistics(
      String teacherId, String name, DateTime start, DateTime end) async {
    _loading.value = true;
                ...
                .then((value) {
                  if (value.docs.isNotEmpty && value.docs.length > 0) {
                    progress = value.docs.map((e) {
                      ProgressModel newProgress = ProgressModel.fromJson(
  ...
  Future<void> getStudentsName() async {
    _loading.value = true;
    await getCurrentUserData();
    await _studentsCollection
        .doc(teacherId)
  ...
  Future pickDateRange(
    ...
    if (name != null) {
      await getStudentStatistics(
          teacherId, name, dateRange.start, dateRange.end);
    }
    update();
  }
